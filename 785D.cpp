...
{
    per[0] = 1;
    for(ll i = 1; i  <= 2000000 + 2; i++)
        per[i] = ((per[i-1] % MOD ) * (i%MOD) ) % MOD;
}
...
ll nCr(ll n, ll r)
{
    ll temp = (per[r] * per[n-r]) % MOD;
    ll ans = (per[n] * BigMod(temp, MOD-2)) % MOD;
    return ans;
}
